<?php
	...
	*/
	...
	$sqlp = "SELECT * FROM $table WHERE admin_id = '$sesid' and semester = '$semester' and year = '$year' and status = 'Passed' and classs = 'Two' and delete_status = 0 ORDER BY gpa DESC, total_mark DESC";
	$exep = mysqli_query($con, $sqlp);
	$resultp = mysqli_num_rows($exep);//End Query Passed

	//Start query for Reffard
	$sqlr = "SELECT * FROM $table WHERE admin_id = '$sesid' and semester = '$semester' and year = '$year' and status = 'Reffard' and classs = 'Two' and delete_status = 0 ORDER BY roll ASC";
	$exer = mysqli_query($con, $sqlr);
	$resultr = mysqli_num_rows($exer);//End Query Reffard

	//Start Query for All student
	$sqla = "SELECT * FROM $table WHERE admin_id = '$sesid' and semester = '$semester' and delete_status = 0 and year = '$year'";
