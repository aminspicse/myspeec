...
//$table = "eight_madrasah";
	...
	if(isset($_GET['searchbtn'])){
		...
		$query = "SELECT * FROM nine_madrasah WHERE admin_id = '$admin_id' and classs = '$class' and department = 'General' and semester = '$semester' and year = '$year' and roll = '$roll' and delete_status = '0'";
...
?>
		...
		<div class="row">
			<div class="col-md-12">
				<h2 class="text-center pra" style="font-size:3vw" ><b><?php echo $ses['school_name']; ?> </b><small><em style="color:green">(for General Update)</em></small> </h2>
			</div>
			<div class="col-md-12">
				<p class="text-center pra" style="font-size:2vw"><?php echo $class.' '.$setting['semester']." Examination ".$setting['year']; ?></p>
			</div>
		</div>
		<?php include '../nine-ten-madrasah/general/general_update.php' ?>
		<form class="form-inline container-fluid" name="" action="" method="post">
			...
			<?php include '../nine-ten-madrasah/general/general_update_subject.php'; ?>
			<input style="display:none" type="text" name="semester_id" value="<?php echo $row['id'] ?>" />
			<input style="display:none" type="text" name="class" value="Nine" />
		</form>
	</body>
</html>