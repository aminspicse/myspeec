<?php
	...
	*/
	...
	$sqlp = "SELECT * FROM $table WHERE admin_id = '$sesid' and semester = '$semester' and year = '$year' and status = 'Passed' and classs = 'Four' and delete_status = '0' ORDER BY gpa DESC, total_mark DESC";
	$exep = mysqli_query($con, $sqlp);
	$resultp = mysqli_num_rows($exep);//End Query Passed

	//Start query for Reffard
	$sqlr = "SELECT * FROM $table WHERE admin_id = '$sesid' and semester = '$semester' and year = '$year' and status = 'Reffard' and classs = 'Four' and delete_status = '0' ORDER BY roll ASC";
