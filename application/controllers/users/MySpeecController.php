<?php 
    class MySpeecController extends CI_Controller
    {
        function __construct()
        {
            parent::__construct();
            $this->load->model('users/MySpeec');
        ...
        function index()
        {
            ...
            //$query['query'] = $this->MySpeec->myspeec();
        ...
        public function myspeec_fetch()
        {
            ...
            $dislike = 0;
            $comment = 0;
            $qry = $this->MySpeec->myspeec($this->input->post('limit'),$this->input->post('start'));
            if($qry->num_rows() > 0){
                        ...
                        <div class="post_data bg-white">
                            <div class="row ">
                                ...
                                <div class="pull-right">
                                    ...
                                    <a href='.base_url('deletepost/'.$row->news_id).'>Delete</a>
                                </div>
                            </div>
        ...
        function edit_post($news_id,$title)
        {
                ...
                $this->load->view('users/profile/profile_leftnav');
                $this->load->view('users/myspeech/edit_post.php', array('query' => $this->MySpeec->Edit_post($news_id), 'error' => '' ));
        ...
        function update_post()
        {
           // $this->form_validation->set_rules('news_title', 'Title', 'required');
                ...
                if($this->MySpeec->Update_post($news, $data) == true)
                {

                // $query['query'] = $this->MySpeec->Edit_post($news_id);
                    ...
                    $this->load->view('users/profile/profile_leftnav');
                    $this->load->view('users/myspeech/edit_post.php', array('query' => $this->MySpeec->Edit_post($news['id']), 'error' => '' ));
        ...
        {
            if($this->MySpeec->Delete_Post($news_id) == true)
            {
